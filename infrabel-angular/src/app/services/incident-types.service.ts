...
export class IncidentTypesService {
  ...
  getTypes(){
    return this._http.get<IncidentTypeData[]>("http://localhost:3000/api/v1/incident_types")
  }

  // Renvoie le type d'incident correspondant à l'id entrée.
  getOneType(id: number){
    return this._http.get<IncidentTypeData[]>("http://localhost:3000/api/v1/incident_types/" + id)
  }

  // Renvoie tous les types dans la langue entrée.
  getTypesByLanguage(language: string){
    return this._http.get<IncidentTypeData[]>("http://localhost:3000/api/v1/incident_types/" + language)
  }
  ...
  getTypeByIDLanguage(id: number, language: string){
    return this._http.get<IncidentTypeData[]>("http://localhost:3000/api/v1/incident_types/" + id + "/" + language)
  }
}
